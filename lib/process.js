 ...
 */
function getFinalImageUrl(cdnPrefix, assetDir, imgPath) {
...
function processPost(data) {
    ...
    if (typeof data.asset_dir !== "undefined" && typeof this.config !== "undefined") {
        ...
        data.content = data.content.replace(imgHtmlReg, (match, imgPath) => {
            ...
            // 拼接完整的 CDN 图片 URL
            let imgUrl = getFinalImageUrl(postUrlPrefix, pathFn.basename(data.asset_dir), imgPath);
            // 返回替换后的原始标签，只替换链接部分
        ...
        data.content = data.content.replace(imgTagReg, (match, type, src) => {
            ...
            // 拼接完整的 CDN 图片 URL
            let imgUrl = getFinalImageUrl(postUrlPrefix, pathFn.basename(data.asset_dir), src);
        ...
        if (data.cover && !isAbsolutePath(data.cover)) {
            ...
            data.cover = getFinalImageUrl(postUrlPrefix, pathFn.basename(data.asset_dir), data.cover);
        }
    }
    return data;
}
